...
and simplify1
  ...
  | TeAbs(x, sigma, body) -> (*\beta*)
    begin 
    match args with 
    | CtxtApp(Scope (subst_app, tsubst_app, (te, _)), rest) -> 
      let v' = simplify (Scope (subst_app, tsubst_app, te)) in 
      let subst = Subst.bind x v' subst in 
      simplify1 rest (Scope (subst, tsubst, body))
    | _ -> 
      let term = simplify2 (Scope (subst, tsubst, term)) in 
      apply term args
    end

  | TeTyAbs(x, body) ->  (*beta_\tau*)
    begin 
    match args with 
    | CtxtTyApp(Scope (subst_tyapp, tsubst_tyapp, (ty, _)), rest) ->
      let ty = Tsubst.apply tsubst_tyapp ty in 
      let tsubst = Tsubst.bind x ty tsubst in
      simplify1 rest (Scope (subst, tsubst, body))
    | _ -> 
      let term = simplify2 (Scope (subst, tsubst, term)) in 
      apply term args
    end
  | TeData (x, tyl, tel, info) -> 
    begin 
    match args with
    | CtxtMatch(Scope (subst_match, tsubst_match, (ty, clauses, _)), rest) -> 
      let Clause (PatData (loc, xp, tyvars, tevars, rest'), body) = List.find (fun (Clause (PatData (_, x', _, _, _), _)) -> Atom.equal x x') clauses in 
      let tsubst_match = List.fold_left2 (fun ts tv ty' -> Tsubst.bind tv (Tsubst.apply tsubst ty') ts) tsubst_match tyvars tyl in 
      let tel' = List.map (fun f -> simplify (Scope (subst, tsubst, f))) tel in  
      let subst_match = List.fold_left2 (fun s tv f -> Subst.bind tv f s) subst_match tevars tel' in 
      simplify1 rest (Scope (subst_match, tsubst_match, body))
    | _ -> 
      let term = simplify2 (Scope(subst, tsubst, term)) in 
      apply term args
    end
...
and simplify2
        (Scope (subst, tsubst, term): fterm scoped): pre_fterm =
  match term with
  | TeLet (x, term1, term2) -> (* inline and drop *)
     let term1' = simplify (Scope (subst, tsubst, term1)) in
     let subst = Subst.bind x term1' subst in 
     simplify (Scope (subst, tsubst, term2))
