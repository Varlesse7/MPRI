...
let rec infer              (* [infer] expects... *)
  ...
  | TeLet (x, term1, body) -> 
    let domain = infer p xenv loc tsubst tenv jenv term1 in 

    let tenv = bind x domain tenv in

    let fty = infer p xenv loc tsubst tenv jenv body in 
      fty

  | TeTyAbs (x, body) -> 

    let domain = infer p xenv loc tsubst tenv jempty body in 
    let fty = TyForall(abstract x domain) in 
    fty
  | TeTyApp (te1, ty, info) -> 
    let domain = infer p xenv loc tsubst tenv jenv te1 in 
    let gen = deconstruct_univ xenv loc domain in 
    info := Some{gen = gen};
    fill gen ty

  | TeData (x, tyl, te, info) -> 
    let xty = lookup_and_instantiate p xenv loc x tyl in 

    let domains, codomain = deconstruct_data_arrow xenv loc x xty (List.length te) in 

    List.iter2(fun term domain -> check p xenv tsubst tenv jenv term domain) te domains;
  ...
  | TeMatch (te1, ty, cl, info) ->  
    ...
    List.iter (check_clause p xenv tsubst tenv jenv ty' ty) cl;
...
and check                  (* [check] expects... *)
  ...
  | TeLoc (loc, term) ->
      ...
      let inferred_expand = TS.apply tsubst inferred in 
      let expected_expand = TS.apply tsubst expected in 
      if not (equal inferred_expand expected_expand) then 
        let stty1 = print_type xenv inferred_expand in 
        let stty2 = print_type xenv expected_expand in 
        let stres = stty1 ^ stty2 in 
        Error.error [loc] ("Not the same type" ^ stres)
